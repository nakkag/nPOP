...
#define HEAD_ENCODE_LINELEN		100
...
/*
 * is_8bit_char - 8bit文字が含まれるかチェック
 */
static BOOL is_8bit_char(char* str)
{
	return (((unsigned char)*str & (unsigned char)0x80) ? TRUE : FALSE);
}
...
char* MIME_body_encode(char* body, TCHAR* charset_t, int encoding, char* ret_content_type, char* ret_encoding, TCHAR* ErrStr)
{
	...
	for (p = buf; *p != TEXT('\0'); p++) {
		if (is_8bit_char(p) == TRUE) {
			encode = TRUE;
			break;
		}
	}
	if (encode == FALSE) {
		MIME_create_encode_header(charset_t, 0, ret_content_type, ret_encoding);
		ret = alloc_copy(buf);
	}
	else {
